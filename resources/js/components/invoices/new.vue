<script setup> 
    ...
    const onSave = () => {
        ...
        if (listCart.value.length >= 1) {
            ...
            const for_data = new formData()
            for_data.append('invoice_item', JSON.stringify(listCart.value))
            for_data.append('customer_id', customer_id.value)
            for_data.append('date', form.value.date)
            for_data.append('due_date',form.value.due_date)
            for_data.append('number', form.value.number)
            for_data.append('reference', form.value.reference)
            for_data.append('discount', form.value.discount)
            for_data.append('subtotal', subtotal)
            for_data.append('total', total)
            for_data.append('term_and_condition', form.value.term_and_condition)
            ...
            <div class="modal__items">
                <ul style="list-style:none">
                    <li v-for=" (item, i) in listProducts " :key="item.id" style="display:grid;grid-template-columns:30px 350px 15px; align-items:center; padding-bottom:5px">
                        ...
                        <button @click="addCart(item)" style="border:1px solid #e0e0e0;width:35px;height:35; cursor:pointer">+</button>
                    </li>
                </ul>
            </div>
            <br><hr>
